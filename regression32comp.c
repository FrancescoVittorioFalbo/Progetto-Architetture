...
void stampaOsservazioni(float*x, float* y, int n, int d){
	for(int i=0;i<n;i++){
		printf("%s\t","[");
		for(int j=0;j<d+1;j++){
			if(j!=d)
...
void stampaXSTAR(float* mat, int n, int d, int deg){
	int l=dim(d,deg)+1;
...
void stampaTheta(float* theta, int d, int deg){
...
void stampaArray(float* theta, int lenght){
	printf("%s\n","");
	int i;
...
extern void adagrad32Prod(int p, int pfin, float* theta, float* osservazioni, int lenght, float* gj, float* Gj, float* y, int i);
extern void aggiornamentoG(int p, int pfin, int lenght, float* sommatoria, float rate, float* Gj, float* gj);
...
float* sgdAdagrad(params* input, int lenght){

	//start sgd
	float* osservazioni=input->xast;
	lenght=dimPadding(lenght);
	float* theta=(float*)get_block(sizeof(float), lenght+100);
	int i;
	float* y=input->y;
	int iter=input->iter;
	int n=input->n;
	...
	float* Gj=(float*)get_block(sizeof(float),k*lenght);
	float* gj=(float*)get_block(sizeof(float),k*lenght);
	float* sommatoria=(float*)get_block(sizeof(float),lenght);
